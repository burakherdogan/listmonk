...
// isBotRequest reports whether a hit on a public tracking endpoint came from a
// mail security scanner, link pre-fetcher or other automated client rather than
// from the subscriber.
func isBotRequest(c echo.Context) bool {
	r := c.Request()
	...
	return bot.IsBot(r.UserAgent(), r.Header.Get("Sec-Purpose"), purpose)
}
...
func (a *App) LinkRedirect(c echo.Context) error {
	...
	url, err := a.core.RegisterCampaignLinkClick(linkUUID, campUUID, subUUID, isBotRequest(c))
	if err != nil {
		e := err.(*echo.HTTPError)
		return c.Render(e.Code, tplMessage, makeMsgTpl(a.i18n.T("public.errorTitle"), "", e.Error()))
	}
...
func (a *App) RegisterCampaignView(c echo.Context) error {
	...
	if campUUID != dummyUUID && subUUID != dummyUUID {
		if err := a.core.RegisterCampaignView(campUUID, subUUID, isBotRequest(c)); err != nil {
